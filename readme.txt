...
**What is Reeview?**
Reeview is the world’s first video delivery system that automatically searches and serves your Youtube product reviews, straight to your shoppers. Control the conversation and keep shoppers on your site with a ground-breaking widget that will revolutionize how you use video.
...
**Features**
If you don't have an account yet here's a link to our website [Reeview](https://reeview.app/).
...
1. Vendor(manufacturer): this configuration provides available attributes that can be set for a product. By selecting one of them will instruct the plugin to consider that attribute as vendor/manufacturer for all products.
Note: Only one value should be selected for each product, if multiple values are selected only the first will be used. If attribute is not set for some products, a null value will be assigned.
2. Enable logs: this configuration is used to decide whether or not to log in a specific file all errors,warnings and notifications that may appear in this plugin. Note: This options should be enabled if you are interested to see that is logged by this plugin.
